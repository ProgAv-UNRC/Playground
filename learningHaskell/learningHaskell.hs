...
{-
## Tipos genéricos como argumentos y restricciones de tipo {#tgen-restrictions}
...
aunque no siempre existe esa definición por defecto. En la sección de [Tipos genéricos como argumentos y restricciones de tipo](#tgen-restrictions) vimos el uso de `... (Eq a) => ...` como una restricción sobre el tipo genérico _a_ que indica _el tipo a debe soportar las operaciones de igualdad (==) y desigualdad (/=)_. _Eq_ es una clase que define las operaciones _==_ y _/=_, esta clase tiene una implementación por defecto que utiliza los constructores utilizados para la comparación de dos valores, por ejemplo `(Succ (Succ Zero)) == (Succ Zero)` compara los nombres de los operadores, primero comparando _Succ_ con _Succ_, y luego comparando _Succ_ con _Zero_, y retornando `False`.
...
En la sección de [Tipos](#types) mencionamos brevemente a las clases de Haskell cuando hablamos del `deriving` en la declaración de un nuevo tipo.
